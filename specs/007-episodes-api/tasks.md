...
- [ ] T001 Confirm the active branch is `007-episodes-api` and the feature directory in `./.specify/feature.json` points to `specs/007-episodes-api`
- [ ] T002 Review and update episode-focused verification shortcuts in `./Makefile` and `./pyproject.toml`
- [ ] T003 [P] Review local environment and manual validation prerequisites for episode sync in `./.env.example` and `specs/007-episodes-api/quickstart.md`
...
- [ ] T005 Create the append-only episode action history ORM model and relationship wiring in `app/db/models/podcast.py`
- [ ] T006 Update ORM exports and metadata imports for the episodes domain in `app/db/models/__init__.py` and `app/db/base.py`
- [ ] T007 Create the Alembic migration for episode action history in `alembic/versions/0005_episode_action_history.py`
- [ ] T008 [P] Add shared episode action request and response schemas in `app/schemas/episode.py` and export them from `app/schemas/__init__.py`
- [ ] T009 [P] Add episode URL sanitization, action-type, playback-progress, and query validation helpers in `app/core/security.py`
- [ ] T010 [P] Extend dependency wiring for episode services in `app/api/deps.py`
- [ ] T011 Implement shared episode-action persistence, projection refresh, and query scaffolding in `app/services/episodes.py`
- [ ] T012 Register the episodes API router in `app/api/routes/episodes_api.py`, `app/api/routes/__init__.py`, `app/services/__init__.py`, and `app/main.py`
...
- [ ] T013 [P] [US1] Create contract tests for `POST /api/2/episodes/{username}.json` in `tests/contract/test_episodes_api.py`
- [ ] T014 [P] [US1] Create unit tests for action validation, URL sanitation, `update_urls`, and projection refresh behavior in `tests/unit/test_episode_service.py`
- [ ] T015 [P] [US1] Create integration tests for mixed upload batches, invalid `play` payloads, and sanitized URL handling in `tests/integration/test_episodes_sync_api.py`

### Implementation for User Story 1

- [ ] T016 [P] [US1] Implement upload normalization, URL rewrite tracking, and append-only event creation in `app/services/episodes.py`
- [ ] T017 [P] [US1] Implement latest-state projection updates for accepted uploads in `app/services/episodes.py` and `app/db/models/podcast.py`
- [ ] T018 [P] [US1] Implement the `POST /api/2/episodes/{username}.json` route in `app/api/routes/episodes_api.py`
- [ ] T019 [US1] Integrate upload response serialization, ownership enforcement, and validation error handling across `app/services/episodes.py`, `app/schemas/episode.py`, `app/core/security.py`, and `app/api/routes/episodes_api.py`
- [ ] T020 [US1] Document manual verification for episode-action uploads in `specs/007-episodes-api/quickstart.md`
...
- [ ] T021 [P] [US2] Extend contract coverage for `GET /api/2/episodes/{username}.json` basic retrieval and `since` behavior in `tests/contract/test_episodes_api.py`
- [ ] T022 [P] [US2] Add unit tests for timestamp issuance, chronological ordering, and `since` filtering in `tests/unit/test_episode_service.py`
- [ ] T023 [P] [US2] Add integration tests for initial retrieval, incremental retrieval, and empty-result responses in `tests/integration/test_episodes_sync_api.py`

### Implementation for User Story 2

- [ ] T024 [P] [US2] Implement episode action history reads, response ordering, and fresh timestamp issuance in `app/services/episodes.py`
- [ ] T025 [P] [US2] Implement the `GET /api/2/episodes/{username}.json` route for unfiltered retrieval in `app/api/routes/episodes_api.py`
- [ ] T026 [US2] Integrate `since` parsing, empty-history responses, and response action serialization across `app/services/episodes.py`, `app/schemas/episode.py`, and `app/api/routes/episodes_api.py`
- [ ] T027 [US2] Document manual verification for full and incremental episode retrieval in `specs/007-episodes-api/quickstart.md`
...
- [ ] T028 [P] [US3] Extend contract coverage for `podcast`, `device`, and `aggregated` query parameters in `tests/contract/test_episodes_api.py`
- [ ] T029 [P] [US3] Add unit tests for podcast filtering, device filtering, aggregation semantics, and deterministic aggregated ordering in `tests/unit/test_episode_service.py`
- [ ] T030 [P] [US3] Add integration tests for podcast-filtered, device-filtered, and aggregated retrieval flows in `tests/integration/test_episodes_sync_api.py`

### Implementation for User Story 3

- [ ] T031 [P] [US3] Implement podcast and device query filtering in `app/services/episodes.py`
- [ ] T032 [P] [US3] Implement `aggregated=true` latest-matching-action reads in `app/services/episodes.py`
- [ ] T033 [P] [US3] Extend the `GET /api/2/episodes/{username}.json` route to support filter and aggregation parameters in `app/api/routes/episodes_api.py`
- [ ] T034 [US3] Integrate query validation, aggregation serialization, and cross-account filter safety across `app/services/episodes.py`, `app/schemas/episode.py`, `app/core/security.py`, and `app/api/routes/episodes_api.py`
- [ ] T035 [US3] Document manual verification for filtered and aggregated episode retrieval in `specs/007-episodes-api/quickstart.md`
...
- [ ] T036 [P] Align implemented behavior with `specs/007-episodes-api/contracts/episodes-api.openapi.yaml`
- [ ] T037 [P] Add any remaining episodes verification required by the constitution in `tests/contract/test_episodes_api.py`, `tests/integration/test_episodes_sync_api.py`, and `tests/unit/test_episode_service.py`
- [ ] T038 [P] Verify that device updates remain compatible with the refreshed latest-state projection in `tests/integration/test_device_updates_api.py` and `tests/unit/test_device_service.py`
- [ ] T039 [P] Remove or refactor any episodes-related inline suppressions encountered while implementing the feature in `app/` and `tests/`
- [ ] T040 Update developer verification shortcuts and feature notes in `./Makefile`, `./README.md`, and `specs/007-episodes-api/quickstart.md`
- [ ] T041 Run and document the full episodes quickstart validation in `specs/007-episodes-api/quickstart.md`
- [ ] T042 Prepare PR summary with implemented scope, verification evidence, migration notes, compatibility notes, and deferred follow-ups in `./PR_SUMMARY.md`
