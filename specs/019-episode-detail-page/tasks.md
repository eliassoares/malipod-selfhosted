...
- [x] T014 [P] [US1] Add integration test for episode detail render and 404 in `tests/integration/test_episode_detail_page.py`
- [x] T015 [P] [US1] Add integration test for progress rendering (position/total) in `tests/integration/test_episode_detail_page.py`

### Implementation for User Story 1

- [x] T016 [P] [US1] Add site router for episode pages in `app/api/routes/episode_site.py`
- [x] T017 [US1] Include the new router in `app/main.py`
- [x] T018 [P] [US1] Create `GET /episode/{episode_id}` handler with locale/copy patterns in `app/api/routes/episode_site.py`
- [x] T019 [P] [US1] Create episode detail template from Stitch reference in `app/templates/episodes/detail.html`
- [x] T020 [P] [US1] Create episode not-found template in `app/templates/episodes/not_found.html`
- [x] T021 [P] [US1] Add localization keys for episode detail UI in `app/core/localization.py`
- [x] T022 [US1] Link episodes list on podcast page to episode detail by wrapping items in `/episode/{id}` in `app/templates/podcasts/detail.html`
...
- [x] T023 [P] [US2] Add integration tests for download visibility + download route behavior in `tests/integration/test_episode_detail_page.py`
- [x] T024 [P] [US2] Add integration test for share link presence in `tests/integration/test_episode_detail_page.py`

### Implementation for User Story 2

- [x] T025 [US2] Add `GET /episode/{episode_id}/download` handler in `app/api/routes/episode_site.py`
- [x] T026 [P] [US2] Add download UI state (hidden/disabled) based on `media_url` in `app/templates/episodes/detail.html`
- [x] T027 [P] [US2] Add share UI that exposes the episode page URL in `app/templates/episodes/detail.html`
- [x] T028 [P] [US2] Add localization keys for download/share UI in `app/core/localization.py`
...
- [x] T029 [P] [US3] Add integration test for favorite toggle persistence in `tests/integration/test_episode_detail_page.py`
- [x] T030 [P] [US3] Add integration test for listening history section (events and empty state) in `tests/integration/test_episode_detail_page.py`

### Implementation for User Story 3

- [x] T031 [US3] Add `POST /episode/{episode_id}/favorite` handler in `app/api/routes/episode_site.py`
- [x] T032 [P] [US3] Add favorite UI controls to the episode template in `app/templates/episodes/detail.html`
- [x] T033 [P] [US3] Add localization keys for favorite/history UI in `app/core/localization.py`
- [x] T034 [US3] Implement history query (recent action events) in `app/services/episode_detail.py`
- [x] T035 [P] [US3] Render history section with empty state in `app/templates/episodes/detail.html`
...
- [x] T036 [P] Ensure mobile responsiveness and basic a11y (labels/sr-only) in `app/templates/episodes/detail.html`
- [x] T037 Run full quality gates and record evidence in PR summary: `uv run pytest`, `uv run ruff check .`, `uv run mypy .`, `uv run bandit -r app -c pyproject.toml`
- [x] T038 Prepare PR summary including scope, verification evidence, and follow-ups in `PR_SUMMARY.md`
