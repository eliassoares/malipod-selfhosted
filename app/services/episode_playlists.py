...
class EpisodePlaylistsService:
    ...
    async def list_cards(self, user: UserModel) -> list[EpisodePlaylistCard]:
        ...
        cards: list[EpisodePlaylistCard] = []
        favorites = await self._build_favorites_card(user)
        cards.append(favorites)
        for playlist in playlists:
            cards.append(await self._build_card_for_playlist(user, playlist))
        return cards
    ...
    async def _build_favorites_card(self, user: UserModel) -> EpisodePlaylistCard:
        ...
        metrics = await self._build_metrics_for_favorite_episodes(user)
        ...
        return EpisodePlaylistCard(
            ...
            listened_seconds=metrics[0],
            total_seconds=metrics[1],
            listened_formatted=format_duration_short(metrics[0]),
            total_formatted=format_duration_short(metrics[1]),
        )

    async def _build_card_for_playlist(
        self, user: UserModel, playlist: EpisodePlaylistModel
    ) -> EpisodePlaylistCard:
        episode_count = await self._count_playlist_items(playlist.id)
        listened_seconds, total_seconds = await self._build_metrics_for_playlist(
    ...
    async def _count_playlist_items(self, playlist_id: int) -> int:
        statement = select(func.count(EpisodePlaylistItemModel.id)).where(
    ...
    async def update_playlist(
        ...
        playlist.image_url = (payload.image_url or "").strip() or None
    ...
    async def add_episode_to_playlists(
        ...
        *,
        ...
        for playlist_id in unique_ids:
            if playlist_id not in allowed_ids:
                continue
            self.session.add(
                EpisodePlaylistItemModel(
                    playlist_id=playlist_id,
    ...
    async def _build_metrics_for_playlist(
        self,
        user: UserModel,
        *,
        playlist_id: int,
        ...
        statement = (
            ...
            .join(
                EpisodePlaylistItemModel,
                EpisodePlaylistItemModel.episode_id == play_index.c.episode_id,
            )
            .where(EpisodePlaylistItemModel.playlist_id == playlist_id)
        )
        rows = (await self.session.execute(statement)).all()
        listened_seconds = sum(int(row.max_pos or 0) for row in rows)
        total_seconds = sum(int(row.max_total or 0) for row in rows)
        return listened_seconds, total_seconds

    async def _build_metrics_for_favorite_episodes(
        self,
        user: UserModel,
    ) -> tuple[int, int]:
        play_index = (
